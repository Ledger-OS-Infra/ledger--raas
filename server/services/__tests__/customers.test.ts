...
describe("CustomerService", () => {
  ...
  it("creates customer, provisions Nomba VA, and persists mapping", async () => {
  ...
  it("maps Nomba validation errors to AppError", async () => {
    vi.mocked(businessExists).mockResolvedValue(true);
  ...
  it("rolls back when persistence fails after Nomba succeeds", async () => {
    vi.mocked(businessExists).mockResolvedValue(true);
    vi.mocked(mockNombaClient.createCustomerVirtualAccount).mockResolvedValue({
      createdAt: "2026-06-28T10:00:00Z",
      accountHolderId: "holder",
